...
import { View, Text, StyleSheet, Platform, Pressable, LayoutAnimation, Animated, ActivityIndicator, TouchableWithoutFeedback } from 'react-native';
...
const ElementoEventoAsociacionAnterior: React.FC<ElementoEventoProps> = ({ evento, borroso, pulsacionLarga, ampliado, setEventoAmpliadoLayout }) => { //Pa que lo que se pase siga de tipo evento
    ...
    return (
        ...
        <TouchableWithoutFeedback  onPressIn={handlePressIn} onPressOut={handlePressOut} onPress={()=>router.navigate({pathname:`/EventosAsociacion/Comentarios`, params:{id_evento:evento?.id_evento, colorFondo : color.colorFondo, colorTexto: color.colorTitulo}}as any)} >
            <Animated.View ref={itemRef} style={[styles.contenedorElemento, {transform: [{ scale: scaleAnim }], height: expandidoAsistencias ? 210 : 205, flexDirection: "column", marginBottom: expandidoAsistencias ? 0 : 17 }]}>
